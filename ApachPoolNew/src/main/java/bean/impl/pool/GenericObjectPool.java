...
import util.PoolUtils;
import bean.assist.IdentityWrapper;
import bean.assist.LinkedBlockingDeque;
import bean.assist.PoolImplUtils;
...
import bean.inter.assist.UsageTracking;
import bean.inter.factory.PooledObjectFactory;
import bean.inter.jmx.GenericObjectPoolMXBean;
import bean.inter.pool.ObjectPool;
import bean.inter.pool.base.BaseGenericObjectPool;
import bean.inter.pool.base.assist.EvictionIterator;
import config.abandon.AbandonedConfig;
import config.evict.EvictionConfig;
import config.evict.EvictionPolicy;
import config.pool.impl.GenericObjectPoolConfig;
...
public class GenericObjectPool<T> extends BaseGenericObjectPool<T> implements ObjectPool<T>, GenericObjectPoolMXBean, UsageTracking<T> {
     ...
     * @param config
     *            The configuration to use for this pool instance. The configuration is used by value. Subsequent changes to the configuration object will not
     *            be reflected in the pool.
     */
     ...
     *            The object factory to be used to create object instances used by this pool
     * @param config
     *            The base pool configuration to use for this pool instance. The configuration is used by value. Subsequent changes to the configuration object
     *            will not be reflected in the pool.
    ...
    /**
     * Returns the cap on the number of "idle" instances in the pool. If maxIdle is set too low on heavily loaded systems it is possible you will see objects
     * being destroyed and almost immediately new objects being created. This is a result of the active threads momentarily returning objects faster than they
     * are requesting them them, causing the number of idle objects to rise above maxIdle. The best value for maxIdle for heavily loaded system will vary but
    ...
    /**
     * Sets the target for the minimum number of idle objects to maintain in the pool. This setting only has an effect if it is positive and
     * {@link #getTimeBetweenEvictionRunsMillis()} is greater than zero. If this is the case, an attempt is made to ensure that the pool has the required
    ...
    /**
     * Returns the target for the minimum number of idle objects to maintain in the pool. This setting only has an effect if it is positive and
     * {@link #getTimeBetweenEvictionRunsMillis()} is greater than zero. If this is the case, an attempt is made to ensure that the pool has the required
     * minimum number of instances during idle object eviction runs.
     * <p>
     * If the configured value of minIdle is greater than the configured value for maxIdle then the value of maxIdle will be used instead.
     ...
     * 
     * @return The abandoned object timeout in seconds if abandoned object removal is configured for this pool; Integer.MAX_VALUE otherwise.
     * 
    ...
    /**
     * Borrow an object from the pool using the specific waiting time which only applies if {@link #getBlockWhenExhausted()} is true.
     * <p>
     * If there is one or more idle instance available in the pool, then an idle instance will be selected based on the value of {@link #getLifo()}, activated
     * and returned. If activation fails, or {@link #getTestOnBorrow() testOnBorrow} is set to <code>true</code> and validation fails, the instance is destroyed
     * and the next available instance is examined. This continues until either a valid instance is returned or there are no more idle instances available.
     * <p>
     * If there are no idle instances available in the pool, behavior depends on the {@link #getMaxTotal() maxTotal}, (if applicable)
     * {@link #getBlockWhenExhausted()} and the value passed in to the <code>borrowMaxWaitMillis</code> parameter. If the number of instances checked out from
     * the pool is less than <code>maxTotal,</code> a new instance is created, activated and (if applicable) validated and returned to the caller. If validation
     * fails, a <code>NoSuchElementException</code> is thrown.
     * <p>
     * If the pool is exhausted (no available idle instances and no capacity to create new ones), this method will either block (if
     * {@link #getBlockWhenExhausted()} is true) or throw a <code>NoSuchElementException</code> (if {@link #getBlockWhenExhausted()} is false). The length of
     * time that this method will block when {@link #getBlockWhenExhausted()} is true is determined by the value passed in to the
     * <code>borrowMaxWaitMillis</code> parameter.
     * <p>
     * When the pool is exhausted, multiple calling threads may be simultaneously blocked waiting for instances to become available. A "fairness" algorithm has
     * been implemented to ensure that threads receive available instances in request arrival order.
     * 
     * @param borrowMaxWaitMillis
     *            The time to wait in milliseconds for an object to become available
     * 
     ...
     * <p>
     * If {@link #getTestOnReturn() testOnReturn} == true, the returning instance is validated before being returned to the idle instance pool. In this case, if
     ...
     * {@link PooledObjectFactory#destroyObject(PooledObject)} method on each idle instance.
     * <p>
     * Implementation notes:
     * <ul>
     * <li>This method does not destroy or effect in any way instances that are checked out of the pool when it is invoked.</li>
     * <li>Invoking this method does not prevent objects being returned to the idle instance pool, even during its execution. Additional instances may be
     * returned while removed items are being destroyed.</li>
     * <li>Exceptions encountered destroying idle instances are swallowed but notified via a {@link SwallowedExceptionListener}.</li>
     * </ul>
     */
    @Override
    public void clear() {
    ...
    /**
     * Closes the pool. Once the pool is closed, {@link #borrowObject()} will fail with IllegalStateException, but {@link #returnObject(Object)} and
     * {@link #invalidateObject(Object)} will continue to work, with returned objects destroyed on return.
     * <p>
     ...
     * 
     * @throws Exception
     *             if the object factory's {@code makeObject} fails
     */
     ...
     * 
     * @throws Exception
     *             If the factory fails to destroy the pooled object cleanly
     */
     ...
     * <p>
     * Creates and adds idle instances until either {@link #getNumIdle()} reaches {@code idleCount} or the total number of objects (idle, checked out, or being
     * created) reaches {@link #getMaxTotal()}. If {@code always} is false, no instances are created unless there are threads waiting to check out instances
     * from the pool.
     * 
     * @param idleCount
     *            the number of idle instances desired
     * @param always
     *            true means create instances even if the pool has no threads waiting
     * @throws Exception
     *             if the factory's makeObject throws
     */
    ...
    /**
     * Add the provided wrapped pooled object to the set of idle objects for this pool. The object must already be part of the pool. If {@code p} is null, this
     * is a no-op (no exception, but no impact on the pool).
     * 
     * @param p
     *            The object to make idle
     * 
     * @throws Exception
     *             If the factory fails to passivate the object
     */
    ...
    /**
     * Return an estimate of the number of threads currently blocked waiting for an object from the pool. This is intended for monitoring only, not for
    ...
    public String getFactoryType() {
        ...
        if (factoryType == null) {
            ...
            result.append('<');
            Class<?> pooledObjectType = PoolImplUtils.getFactoryType(factory.getClass());
     ...
     * <p>
     * Note: This is named listAllObjects so it is presented as an operation via JMX. That means it won't be invoked unless the explicitly requested whereas all
     * attributes will be automatically requested when viewing the attributes for an object in a tool like JConsole.
     * 
    ...
    /*
     * All of the objects currently associated with this pool in any state. It excludes objects that have been destroyed. The size of {@link #allObjects} will
     * always be less than or equal to {@link #_maxActive}. Map keys are pooled objects, values are the PooledObject wrappers used internally by the pool.
     */
    private final Map<IdentityWrapper<T>, PooledObject<T>> allObjects = new ConcurrentHashMap<IdentityWrapper<T>, PooledObject<T>>();
    /*
     * The combined count of the currently created objects and those in the process of being created. Under load, it may exceed {@link #_maxActive} if multiple
     * threads try and create a new object at the same time but {@link #create()} will ensure that there are never more than {@link #_maxActive} objects created
     * at any one time.
     */
